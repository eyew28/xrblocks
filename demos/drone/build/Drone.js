...
export class Drone extends xb.Script {
    ...
    updateRigidBody() {
        ...
        const controllerForce = new THREE.Vector3(this.keyboardControls.getDroneRightForce(), this.keyboardControls.getDroneUpForce(), this.keyboardControls.getDroneForwardForce());
        ...
        if (leftControllerAxes && leftControllerAxes.length >= 4) {
            rotation.premultiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), -this.rotationSpeed * leftControllerAxes[2]));
    ...
    resetDrone() {
        ...
        xb.extractYaw(this.quaternion.setFromRotationMatrix(camera.matrixWorld), this.quaternion);
